/**
 * @file Merkle.c
 * @brief Implementation of a Merkle tree using SHA-256 and a queue-based approach.
 *
...
typedef struct queue_element_t {
  ...
  int queue_element_type; /**< Type of the queue element (DATA_TYPE or HASH_TYPE). */
} queue_element_t;
 ...
 */
static merkle_error_t hash_data_block(const void *data, size_t size, unsigned char out[HASH_SIZE]);
 ...
 * @return MERKLE_SUCCESS on success, error code otherwise.
 */
static merkle_error_t hash_two_nodes(merkle_node_t *n1, merkle_node_t *n2, unsigned char out[HASH_SIZE]);
 ...
 * @return MERKLE_SUCCESS on success, error code otherwise.
 */
static merkle_error_t build_tree_from_queue(queue_t *queue, merkle_tree_t **result);
 ...
 */
merkle_tree_t *merkle_tree_create(const void **data, const size_t *size, size_t count);
  ...
  *result = MMalloc(sizeof *result);
  ...
  while (get_queue_size(queue)) {
    ...
    if (element->queue_element_type == DATA_TYPE) {
      ...
      ret_code = hash_data_block(element->data.d, element->data.size, (h_el->mnode->hash));
      ...
      if (!get_queue_size(queue)) {
        ...
        ret_code = hash_two_nodes(element->mnode, next_element->mnode, &(out_hash[0]));
        if (ret_code != MERKLE_SUCCESS) {
